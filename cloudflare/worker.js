// ═══════════════════════════════════════════════════════════════
// ADIZA MUNOWATCH WORKER — Uganda VJ Movies + License System
// Merged: Uganda Munowatch routes + Adiza Moviez Box license/activation
// ═══════════════════════════════════════════════════════════════

// ── All globals set from env bindings on each request ──
let BOT_TOKEN = '', ADMIN_SECRET = '', GROUP_ID_DEFAULT = 0, CHANNEL_ID = 0;
let MUNO_EMAIL = '', MUNO_PASS = '', MUNO_API_KEY = '';
let APP_HMAC_SECRET = '', RESP_SIGN_SECRET = '';

// ── Munowatch URL constants (not secret) ────────────────────────
const MUNO_COM  = "https://munowatch.com";
const MUNO_ORG  = "https://munowatch.org";
const MUNO_UA   = "Mozilla/5.0 (Linux; Android 13; SM-G991B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36";

let _munoSession = null; // { cookie, expires }
...
async function munoGetSession() {
  ...
  const resp = await fetch(`${MUNO_COM}/login`, {
    ...
    headers: {
      ...
      "User-Agent": MUNO_UA, "Referer": `${MUNO_COM}/login`, "Accept": "text/html,application/xhtml+xml",
    },
  ...
  if (!m) throw new Error("Munowatch login failed — no session cookie");
  const cookie = `PHPSESSID=${m[1]}`;
  _munoSession = { cookie, expires: Date.now() + 25 * 60 * 1000 };
  return cookie;
}

async function munoAuthedGet(path) {
  let cookie = await munoGetSession();
  let resp = await fetch(`${MUNO_COM}${path}`, {
    headers: { "User-Agent": MUNO_UA, "Accept": "text/html,application/xhtml+xml", "Referer": MUNO_COM, "Cookie": cookie },
    redirect: "manual",
  });
  if (resp.status === 302) {
    _munoSession = null;
...
function munoParseMovies(html) {
  const movies = [];
  const parts = html.split(/href="\/twolekede\?/);
...
async function getGroupId(env) {
  // Hardcoded value is authoritative; DB setting used only as override if explicitly different
...
async function initDB(db) {
  // Legacy table (DEX v1 backward compat — device_id PK)
  ...
  // NEW: Multi-app activations (app_id + device_id composite key)
  ...
  // NEW: App-aware tokens (add app_id column to vip_tokens if missing)
  try { await db.exec("ALTER TABLE vip_tokens ADD COLUMN app_id TEXT NOT NULL DEFAULT 'default'"); } catch (e) {}
  try { await db.exec("ALTER TABLE apps ADD COLUMN app_secret TEXT NOT NULL DEFAULT ''"); } catch (e) {}
  try { await db.exec("ALTER TABLE apps ADD COLUMN dialog_variant TEXT NOT NULL DEFAULT 'reversal'"); } catch (e) {}
  try { await db.exec("ALTER TABLE apps ADD COLUMN pkg_suffix TEXT NOT NULL DEFAULT ''"); } catch (e) {}
  try { await db.exec("ALTER TABLE apps ADD COLUMN package_name TEXT NOT NULL DEFAULT ''"); } catch (e) {}
  try { await db.exec("ALTER TABLE app_activations ADD COLUMN expired_notified INTEGER NOT NULL DEFAULT 0"); } catch (e) {}
  try { await db.exec("ALTER TABLE app_activations ADD COLUMN telegram_user_id INTEGER NOT NULL DEFAULT 0"); } catch (e) {}
  try { await db.exec("ALTER TABLE app_activations ADD COLUMN warn_sent INTEGER NOT NULL DEFAULT 0"); } catch (e) {}
  try { await db.exec("ALTER TABLE app_activations ADD COLUMN source TEXT NOT NULL DEFAULT 'telegram'"); } catch (e) {}
  await db.exec("CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT NOT NULL DEFAULT '')");
  // Pending activations — stores user-submitted device IDs awaiting admin approval
...
function isNearExpiry(expiry, ms = 48 * 3600 * 1000) {
  if (!expiry || expiry === '2080' || expiry === '2099-12-31') return false;
  try { const exp = new Date(expiry).getTime(); const now = Date.now(); return exp > now && (exp - now) <= ms; } catch { return false; }
}
function parseExpiry(str) {
  ...
  if (mo) { const d = new Date(); d.setMonth(d.getMonth() + +mo[1]); return d.toISOString(); }
  if (wk) return new Date(Date.now() + +wk[1]*7*86400000).toISOString();
  if (dy) return new Date(Date.now() + +dy[1]*86400000).toISOString();
  if (hr) return new Date(Date.now() + +hr[1]*3600000).toISOString();
  if (mn) return new Date(Date.now() + +mn[1]*60000).toISOString();
  return str;
}
function fmtExpiry(exp) {
  if (!exp || exp === '2080' || exp === '2099-12-31') return '♾️ Lifetime';
  try { return new Date(exp).toUTCString().replace(' GMT',' UTC'); } catch { return exp; }
}
...
async function verifyRequestHmac({ device_id, nonce, ts, sig, pkg }) {
  if (!APP_HMAC_SECRET) return true;  // fail-open if admin hasn't set key yet
  if (!nonce || !ts || !sig) return false;
  const now = Math.floor(Date.now() / 1000);
  if (Math.abs(now - Number(ts)) > 300) return false; // reject requests >5 min old
...
// Returns true only when the provided secret exactly matches the app's stored secret.
// Empty secret in DB = app not properly configured = DENY (fail-closed security).
function validateAppSecret(app, provided) {
  if (!app) return false;
  const stored = app.app_secret && app.app_secret.trim() !== '' ? app.app_secret : '';
  if (!stored) return false; // fail-closed: no secret configured = deny
...
function html(content, status=200) { return new Response(content, { status, headers: { 'Content-Type':'text/html; charset=utf-8', ...CORS } }); }
function ok() { return new Response('ok', { status: 200 }); }

// ── Bold text converter (Mathematical Sans-Serif Bold) ────────────
function toBold(text) {
  return [...text].map(c => {
    const code = c.codePointAt(0);
    if (code >= 65 && code <= 90) return String.fromCodePoint(code + 120211);  // A-Z → 𝗔-𝗭
    if (code >= 97 && code <= 122) return String.fromCodePoint(code + 120205); // a-z → 𝗮-𝘇
    return c;
  }).join('');
}
...
function deviceKb(deviceId, appId, isActive) {
  ...
  const toggleBtn = isActive
    ? { text: '⛔ Revoke',  callback_data: `rv:${safe}` }
    : { text: '✅ Restore', callback_data: `rs:${safe}` };
  return { inline_keyboard: [
    [ toggleBtn, { text: '🔄 Refresh', callback_data: `ck:${safe}` } ],
    [ { text: '🗑️ Delete',  callback_data: `dl:${safe}` } ],
  ]};
}

// ── Announcement banner rotation (6 banners, cycles 1→2→3→4→5→6→1) ──
const ANNOUNCE_PHOTOS = [
  'AgACAgQAAxkDAAN-agE0Hd3_ewpE64VAuF-FrptXjIsAAssNaxtWiwlQgi0aWMZoLecBAAMCAAN3AAM7BA', // 1 cyberpunk
  'AgACAgQAAxkDAAOCagE0OlMKm7sbBrkFij5xtOOAubMAAs8NaxtWiwlQ_9iBKWlD39sBAAMCAAN3AAM7BA', // 2 ice
  'AgACAgQAAxkDAAODagE0PN-71pRknetQL9fI9JHlzkoAAtANaxtWiwlQo9SsYYJwlZYBAAMCAAN3AAM7BA', // 3 neon
  'AgACAgQAAxkDAAOEagE0cmBWnK5SuWfIa2IPQRIri7wAAtENaxtWiwlQ2A8XOueE_VQBAAMCAAN3AAM7BA', // 4 lava
  'AgACAgQAAxkDAAOAagE0IPbxh-MJnxdY3wE522TGu2YAAs0NaxtWiwlQ4tQAAdyaiH9ZAQADAgADdwADOwQ', // 5 matrix
  'AgACAgQAAxkDAAOBagE0IuSSUm7EcMUpYdjyvhQ9VjMAAs4NaxtWiwlQoa2sH2Govj0BAAMCAAN3AAM7BA', // 6 purple
];
async function getNextBannerPhoto(db) {
  ...
  } catch { return ANNOUNCE_PHOTOS[0]; }
}

// ── Announcement inline keyboard ──────────────────────────────────
const ANNOUNCE_KB = { inline_keyboard: [
  [{ text: '🪀 Join our Channel 🪀', url: 'https://t.me/reversemoda' }],
  [{ text: '🦋 Reversal_X_Chatroom 🦋', url: 'https://t.me/reversalxmods1' }]
] };
...
function buildExpiredMessage(username, expiry, appName) {
  let expiryDisplay;
  try { expiryDisplay = new Date(expiry).toUTCString().replace(' GMT', ''); } catch { expiryDisplay = expiry || '—'; }
  return `@${username}
...
└ ${toBold('Expired On')} : 🧭  ${expiryDisplay} 🧭

📥 Send a message to the admin in the group to get renewed.
...
🦋 ${isResend ? toBold('ALREADY ACTIVE') + ' 🦋' : toBold('DEVICE ACTIVATED') + ' 🦋'}

Your device has been verified and activated successfully. Kindly close your app and open again 😊

├ ${toBold('Status')} : ✅  ${toBold('Active')}  ✅

└ ${toBold('Expiry')} : 🧭  ${expiryDisplay} 🧭

📥 Enjoy full VIP access with no restrictions.
...
function buildDMMessage(username, app) {
  const cmd = app ? app.tg_command : 'appname';
  const appName = app ? app.display_name : 'the app';
  return `🛡️ <b>REVERSAL X MODS — VIP ACCESS</b>\n━━━━━━━━━━━━━━━━━━━━━━\n\n👋 Hey <b>${username}</b>!\n\n✅ <b>How to Get Activated:</b>\n\n<b>1️⃣</b> Open <b>${appName}</b>\n      → Your <b>Device ID</b> appears on screen\n\n<b>2️⃣</b> Send it here or in the group:\n      <code>/${cmd} YOUR_DEVICE_ID</code>\n\n<b>3️⃣</b> Admin activates you — app unlocks automatically 🎉\n\n━━━━━━━━━━━━━━━━━━━━━━\n<i>⚡ Powered by Reversal X Mods</i>`;
}
...
function buildAdminHelp(apps) {
  ...
  const cmds = apps.length
    ? apps.map(a =>
        `• <code>/${a.tg_command} &lt;device_id&gt;</code>  ← user self-registers\n` +
  ...
  const text =
    `🔐 <b>𝗧𝗲𝗹𝗲𝗣𝗮𝘁 𝗣𝗮𝗻𝗲𝗹 — Admin Commands</b> 🔐\n━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    `🆕 <b>𝗔𝗣𝗣 𝗖𝗢𝗠𝗠𝗔𝗡𝗗𝗦</b>\n${cmds}\n` +
    `  ↳ e.g. <code>/${exCmd} 79837377 RX-ABC123 30d</code>\n\n` +
    `🏆 <b>𝗔𝗖𝗧𝗜𝗩𝗔𝗧𝗜𝗢𝗡 / 𝗥𝗘𝗡𝗘𝗪𝗔𝗟</b>\n` +
  ...
  const kb = { inline_keyboard: [
    ...
    [
      ...
      { text: '📂 List All',   callback_data: 'cmd:listall' },
    ],
  ]};
...
// ── VIP HTML constants (exact reference design) ──────────────────
...
// ── Dialog HTML builder (exact reference design) ──────────────────
...
// ── Direct VIP dialog HTML builder (package-name based, no Telegram bot) ────
function buildDirectDialogHTML(deviceId, appId, appName, pkg, adminLink) {
  ...
  const DIRECT_STYLE = BASE_STYLE
    .replace(/rgba\(201,0,0,/g, 'rgba(14,165,233,')
    .replace(/#c90000/g, '#0ea5e9')
    .replace(/#8b0000/g, '#0284c7');
  return `<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">
<title>${_name}</title>
...
// ── Matrix dialog HTML builder (no-Telegram variant) ─────────────
function buildMatrixDialogHTML(deviceId, workerUrl, appId, appName, groupLink, appSecret) {
  const _appId  = appId     || '';
  const _secret = appSecret || '';
  const _link   = groupLink || 'https://t.me/+rvI8wgFwCiA3NWJk';
  const MATRIX_STYLE = BASE_STYLE
    .replace(/rgba\(201,0,0,/g, 'rgba(14,165,233,')
    .replace(/#c90000/g, '#0ea5e9')
    .replace(/#8b0000/g, '#0284c7');
  return `<!DOCTYPE html><html lang="en"><head>
...
// ── Group rejoin handler ──────────────────────────────────────────
// Called when a user joins/rejoins the Telegram group.
// Looks up all activations for that telegram_user_id, then announces
// their restored access by tagging them in the group.
async function handleGroupRejoin(user, env, token, groupId) {
  if (!user?.id) return;
  const userId   = user.id;
  const uname    = user.username ? `@${user.username}` : (user.first_name || 'User');

  // Find all active app_activations for this Telegram user
  const rows = (await env.DB.prepare(
    "SELECT a.app_id, a.device_id, a.expiry, a.is_active, ap.display_name " +
    "FROM app_activations a LEFT JOIN apps ap ON a.app_id = ap.app_id " +
    "WHERE a.telegram_user_id = ? AND a.is_active = 1"
  ).bind(userId).all()).results || [];

  if (!rows.length) return; // Not a registered user — ignore

  const appNames = [...new Set(rows.map(r => r.display_name || r.app_id))].join(', ');

  const msg =
`✅ *Welcome back, ${uname}\\!*

Your *${appNames}* access has been *restored* 🎉

Close the app and open it again — everything should work normally now\\.

_You must stay in this group to keep your access\\._`;
...
async function handleTelegram(request, env) {
  ...
  // ── Group join detection (chat_member update — most reliable) ────────────
  // Telegram sends this when a user's status changes in the group.
  // We handle it when someone becomes a member after having left.
  if (update.chat_member) {
    const cm = update.chat_member;
    const newStatus  = cm.new_chat_member?.status || '';
    const oldStatus  = cm.old_chat_member?.status || '';
    const joinedUser = cm.new_chat_member?.user;
    const joinChatId = cm.chat?.id;
    const rejoined   = ['member','administrator'].includes(newStatus) &&
                       ['left','kicked','banned'].includes(oldStatus);
    if (rejoined && joinedUser && joinChatId === groupId) {
      await handleGroupRejoin(joinedUser, env, token, groupId);
    }
    return ok();
  }
  ...
  // ── new_chat_members — fallback join detection via service message ────────
  if (msg.new_chat_members && chatId === groupId) {
  ...
  if (chatType === 'private') {
    if (isAdmin) {
      // Admin in DM — try app-command format first, then standard admin commands
      if (text.startsWith('/')) {
        const parts = text.trim().split(/\s+/);
        const cmdRaw = parts[0].slice(1).toLowerCase().split('@')[0];
        const args = parts.slice(1);
        // /<app_command> <user_id> <device_id> [expiry]  — admin verified activation
        if (args.length >= 2 && /^\d+$/.test(args[0])) {
          const app = await env.DB.prepare('SELECT * FROM apps WHERE tg_command=?').bind(cmdRaw).first();
          if (app) {
      ...
      // Non-admin in DM — total silence, no reply, no activation
      return ok();
    }
    return ok();
  }

  // Admin commands — only in the configured group
  if (chatId === groupId && isAdmin && /^\/(activate|revoke|restore|delete|list|listall|check|stats|help|pending|clearapp|renewall|clear_database)/i.test(text)) {
    await handleAdminCommand(chatId, text, env, token, groupId, adminIds, fromId);
    return ok();
  }

  // Dynamic app commands — only in the configured group
  if (chatId === groupId && (chatType === 'group' || chatType === 'supergroup') && text.startsWith('/')) {
    const parts = text.trim().split(/\s+/);
    const cmdRaw = parts[0].slice(1).toLowerCase().split('@')[0];
    const args = parts.slice(1);
    ...
    // Admin: /<app_command> <user_id> <device_id> [expiry] — only in configured group
    if (chatId === groupId && isAdmin && args.length >= 2 && /^\d+$/.test(args[0])) {
      await handleAdminAppActivation(chatId, app, args[0], args[1], args[2] || null, env, token);
      return ok();
    }

    // Auto-activation (token flow): device_id.token
    if (args.length === 1 && args[0].includes('.')) {
      await handleActivationRequest(chatId, args[0], username, fromId, app, env, token);
      return ok();
    }

    // User registration: /<app_command> <device_id>
    if (args.length === 1) {
      await handleUserRegistration(chatId, args[0], username, fromId, app, env, token, groupId);
      return ok();
    }
  }
  return ok();
...
The code looks like: <code>abc123.def456</code>`, 'HTML');
    return;
  }
  ...
  // ── Expired account guard — check BEFORE token validation ─────────────────
  // If this Telegram user already has an expired subscription, send the proper
  // expiry template instead of a generic "invalid code" error.
  if (fromId) {
...
└ ${toBold('Expired On')}   : 🧭  ${fmtExpiry(expiredAct.expiry)} 🧭

📥 Contact the admin to renew your existing subscription.
...
Open the VIP screen again to generate a new one.`);
  ...
  if (Date.now() - tokenRow.created_at > 10 * 60 * 1000) {
...
This code belongs to a different device. Copy it directly from your own VIP screen.`);
  ...
  // Check if this @username already has an activation for this app
...
└ ${toBold('Expired On')}   : 🧭  ${fmtExpiry(existingByUser.expiry)} 🧭
...
Each account can only activate one device per app. Contact the admin if you need to switch devices.`);
  ...
  // Check if this device is already activated for this app by someone else
...
// ── User submits device ID for admin to verify & activate ──────────
async function handleUserRegistration(chatId, deviceId, username, fromId, app, env, token, adminGroupId) {
  ...
  // Already activated by this device?
  ...
  // Block expired users — they MUST contact admin to renew, cannot self-re-register
  ...
  // ── Anti-cheat: same Telegram user already has a DIFFERENT device for this app ──
  const existingByUser = await env.DB.prepare(
...
➖➖➖➖➖➖➖➖➖`);
  ...
  // Auto-activate immediately — no pending, no admin approval needed
...
// ── Admin activates a verified user ────────────────────────────────
// Format: /<app_command> <telegram_user_id> <device_id> [expiry]
// Admin directly activates — no prior submission needed.
// Binds device_id ↔ telegram_user_id so one device can't serve two accounts (anti-cheat).
...
<code>/${escHtml(app.tg_command)} 123456789 DEVICE_ID</code>`, 'HTML');
    return;
  }
  if (clean.length < 4) {
  ...
  // ── Anti-cheat 1: Device already bound to a DIFFERENT user → hard block ──
...
📱 Device: <code>${escHtml(clean)}</code>
👤 Already registered to: @${escHtml(deviceBound.telegram_username)} (<code>${deviceBound.telegram_user_id}</code>)

One device cannot be shared between two accounts.
Revoke the other user first, then re-activate.`, 'HTML');
    return;
  }

  // ── Anti-cheat 2: User already has a DIFFERENT device → deactivate old one ──
  ...
  // ── Resolve username from pending submissions or past records ──
  ...
  // ── Store activation with permanent device ↔ user_id binding ──
  await env.DB.prepare(
    "INSERT OR REPLACE INTO app_activations (app_id, device_id, telegram_username, telegram_user_id, expiry, is_active, created_at) VALUES (?,?,?,?,?,1,?)"
  ).bind(app.app_id, clean, tgUser, userId, expiry, Date.now()).run();

  // Clear any pending request for this user
  await env.DB.prepare(
    "DELETE FROM pending_activations WHERE telegram_user_id=? AND app_id=?"
  ).bind(userId, app.app_id).run();

  // ── Send the full user-facing activation template — mentions USER, not admin ──
  await sendMessage(token, chatId, buildActivationMessage(tgUser, clean, expiry, app.display_name, false));

  // Note if old device was swapped out
  if (userOldDevice) {
    await sendMessage(token, chatId,
`🔄 <i>Note: old device <code>${escHtml(userOldDevice.device_id.slice(0,14))}…</code> has been deactivated for this account.</i>`, 'HTML');
  }
}
...
async function handleCallbackQuery(cb, env, token, groupId, adminIds) {
  ...
  if (!adminIds.includes(fromId)) { await answerCallback(token, cb.id, '⛔ Admin only.'); return; }
  if (data === 'noop') { await answerCallback(token, cb.id); return; }
  ...
  if (ckM) {
    ...
    await answerCallback(token, cb.id, '🔍 Refreshed');
    return;
  }
  ...
  if (rvM) {
    ...
    } catch (e) { await answerCallback(token, cb.id, `Error: ${e.message}`); }
    return;
  }
  ...
  if (dlcM) {
    try {
      ...
      await editMessageText(token, chatId, msgId, `🗑️ <b>DEVICE DELETED</b>\n\n📦 App: ${dlcM[1]}\n📱 <code>${dlcM[2]}</code>\n\n💀 <i>Permanently removed.</i>`, 'HTML');
      await answerCallback(token, cb.id, '🗑️ Deleted');
    } catch (e) { await answerCallback(token, cb.id, `Error: ${e.message}`); }
    return;
  }
  // clearapp:select — show confirmation for a specific app
  const caM = /^clearapp:select:(.+)$/.exec(data);
  if (caM) {
    try {
      const appId = caM[1];
      const app = await env.DB.prepare("SELECT display_name FROM apps WHERE app_id=?").bind(appId).first();
  ...
  if (cacM) {
    try {
      ...
      await editMessageText(token, chatId, msgId,
        `✅ <b>APP DATA CLEARED</b>\n\n📦 App: <b>${escHtml(name)}</b>\n👥 Users removed: <b>${r1.changes || 0}</b>\n🔑 Tokens removed: <b>${r2.changes || 0}</b>\n\n✔️ All other apps are untouched.`,
        'HTML');
  ...
  // ── Quick-action buttons from /start welcome & help keyboard ────
  if (data === 'cmd:stats') {
...
Press <b>WIPE DATABASE</b> to confirm.`, 'HTML', kb);
  ...
  if (data === 'clrdb:yes') {
    try {
      ...
      await editMessageText(token, chatId, msgId,
`🗑️ <b>DATABASE WIPED</b>
━━━━━━━━━━━━━━━━━━
💀 <b>${total}</b> total record(s) permanently deleted.

👥 Users removed: <b>${users}</b>
🔑 Tokens removed: <b>${tokens}</b>
⏳ Pending removed: <b>${pending}</b>

The database is now empty. All users must re-activate.`, 'HTML');
...
`❌ <b>Cancelled</b>
━━━━━━━━━━━━━━━━━━
No changes were made. The database is intact.`, 'HTML');
    await answerCallback(token, cb.id, '❌ Cancelled');
    return;
  }
...
async function sendCheckInfo(token, chatId, appId, deviceId, env, msgIdToEdit) {
  try {
    ...
    if (deviceId.startsWith('@')) {
      ...
      row = appId === 'all'
        ? await env.DB.prepare("SELECT * FROM app_activations WHERE telegram_username=? ORDER BY created_at DESC").bind(uname).first()
        : await env.DB.prepare("SELECT * FROM app_activations WHERE app_id=? AND telegram_username=?").bind(appId, uname).first();
    } else {
      row = appId === 'all'
        ? await env.DB.prepare("SELECT * FROM app_activations WHERE device_id=? ORDER BY created_at DESC").bind(deviceId).first()
        : await env.DB.prepare("SELECT * FROM app_activations WHERE app_id=? AND device_id=?").bind(appId, deviceId).first();
    }
    if (!row) {
...
`🔍 <b>DEVICE INFO</b>
━━━━━━━━━━━━━━━━━━
📦 App: <b>${escHtml(row.app_id)}</b>
👤 User: @${escHtml(row.telegram_username || 'N/A')}
📱 Device: <code>${escHtml(row.device_id)}</code>
📅 Expiry: <b>${fmtExpiry(row.expiry)}</b>
🏷️ Status: <b>${status}</b>
🗓️ Since: ${row.created_at ? new Date(row.created_at).toLocaleDateString('en-GB') : 'N/A'}`;
  ...
  } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
}
...
async function sendDeviceList(token, chatId, appId, page, env, msgIdToEdit, detailed = false) {
  ...
  try {
    const query = appId === 'all'
      ? await env.DB.prepare("SELECT * FROM app_activations ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(limit, offset).all()
      : await env.DB.prepare("SELECT * FROM app_activations WHERE app_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?").bind(appId, limit, offset).all();
    const countQ = appId === 'all'
      ? await env.DB.prepare("SELECT COUNT(*) as total FROM app_activations").first()
      : await env.DB.prepare("SELECT COUNT(*) as total FROM app_activations WHERE app_id=?").bind(appId).first();
    const total = countQ?.total || 0;
    const pages = Math.ceil(total / limit) || 1;
    const results = query.results || [];
    if (!results.length) {
      const t = '📋 No activations found.';
      if (msgIdToEdit) await editMessageText(token, chatId, msgIdToEdit, t);
      else await sendMessage(token, chatId, t);
      return;
    }
    const title = detailed
      ? `📂 <b>ALL ACTIVATIONS${appId !== 'all' ? ` — ${escHtml(appId)}` : ''}</b>`
      : `📋 <b>DEVICES${appId !== 'all' ? ` — ${escHtml(appId)}` : ''}</b>`;
    let msg = `${title}  (Page ${page}/${pages} · Total: ${total})\n━━━━━━━━━━━━━━━━━━\n\n`;
    results.forEach((r, i) => {
      const st = statusBadge(r.is_active, r.expiry);
      const date = r.created_at ? new Date(r.created_at).toLocaleDateString('en-GB') : 'N/A';
      if (detailed) {
        msg += `<b>${offset+i+1}.</b> @${escHtml(r.telegram_username||'N/A')} [${escHtml(r.app_id)}] — ${st}\n<code>${escHtml(r.device_id)}</code>\n📅 ${fmtExpiry(r.expiry)} · 🗓️ ${date}\n\n`;
      } else {
        msg += `<b>${offset+i+1}.</b> @${escHtml(r.telegram_username||'N/A')} [${escHtml(r.app_id)}] — ${st}\n<code>${escHtml(r.device_id)}</code> · ${fmtExpiry(r.expiry)}\n\n`;
      }
    });
    const navRow = [];
    ...
    const toggleRow = detailed
      ? [{ text: '📋 Basic List',    callback_data: `ls:${appId}:1`  }]
      : [{ text: '📂 Detailed List', callback_data: `lsa:${appId}:1` }];
...
async function handleAdminCommand(chatId, text, env, token, groupId, adminIds, fromId) {
  if (/^\/start/i.test(text)) {
    const kb = { inline_keyboard: [
      ...
      [
        { text: '📋 List Users', callback_data: 'cmd:list'    },
        { text: '📂 List All',   callback_data: 'cmd:listall' },
      ],
      [
        { text: '📖 Commands',   callback_data: 'cmd:help'    },
      ],
    ]};
    await sendMessageKeyboard(token, chatId,
      `🔐 <b>𝗧𝗲𝗹𝗲𝗣𝗮𝘁 𝗣𝗮𝗻𝗲𝗹</b> 🔐\n━━━━━━━━━━━━━━━━━━━━━━\n\n` +
      `👋 Welcome back, admin!\n\n` +
  ...
  if (/^\/help/i.test(text)) {
    const apps = (await env.DB.prepare('SELECT * FROM apps').all()).results || [];
  ...
  if (actM) {
    ...
    try {
      ...
      if (existing) {
        await env.DB.prepare("UPDATE app_activations SET device_id=?, expiry=?, is_active=1, created_at=? WHERE app_id=? AND telegram_username=?")
          .bind(deviceId, expiry, Date.now(), appId, tgUser).run();
      } else {
        await env.DB.prepare("INSERT OR REPLACE INTO app_activations (app_id, device_id, telegram_username, telegram_user_id, expiry, is_active, created_at) VALUES (?,?,?,?,?,1,?)")
          .bind(appId, deviceId, tgUser, 0, expiry, Date.now()).run();
      }
      const kb = deviceKb(deviceId, appId, true);
      await sendMessageKeyboard(token, chatId,
`🏆 <b>ACTIVATED</b>\n━━━━━━━━━━━━━━━━━━\n📦 App: <b>${appId}</b>\n👤 User: @${tgUser}\n📱 Device: <code>${deviceId}</code>\n📅 Expiry: <b>${fmtExpiry(expiry)}</b>\n✅ Status: <b>Active</b>`, 'HTML', kb);
    } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
    return;
  }

  // /revoke <app_id> <device_id>
  const rvM = /^\/revoke\s+(\S+)\s+(\S+)/i.exec(text);
  if (rvM) {
    try {
      await env.DB.prepare("UPDATE app_activations SET is_active=0 WHERE app_id=? AND device_id=?").bind(rvM[1], rvM[2]).run();
      await sendMessageKeyboard(token, chatId,
`⛔ <b>REVOKED</b>\n━━━━━━━━━━━━━━━━━━\n📦 App: <b>${rvM[1]}</b>\n📱 Device: <code>${rvM[2]}</code>`, 'HTML', deviceKb(rvM[2], rvM[1], false));
    } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
    return;
  }

  // /restore <app_id> <device_id>
  const rsM = /^\/restore\s+(\S+)\s+(\S+)/i.exec(text);
  if (rsM) {
    try {
      await env.DB.prepare("UPDATE app_activations SET is_active=1 WHERE app_id=? AND device_id=?").bind(rsM[1], rsM[2]).run();
      await sendMessageKeyboard(token, chatId,
`✅ <b>RESTORED</b>\n━━━━━━━━━━━━━━━━━━\n📦 App: <b>${rsM[1]}</b>\n📱 Device: <code>${rsM[2]}</code>`, 'HTML', deviceKb(rsM[2], rsM[1], true));
    } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
    return;
  }

  // /delete <app_id> <device_id>
  ...
  if (dlM) {
    try {
      ...
      await sendMessage(token, chatId, `🗑️ <b>DELETED</b>\n📦 App: <b>${dlM[1]}</b>\n📱 <code>${dlM[2]}</code>`, 'HTML');
    } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
    return;
  }

  // /renewall <app_id> [expiry]  — bulk-renew every active user on an app
  const raM = /^\/renewall\s+(\S+)(?:\s+(\S+))?/i.exec(text);
  if (raM) {
    const [, appId, rawExp] = raM;
    const app = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(appId).first();
    if (!app) {
    ...
    try {
      ...
      await sendMessage(token, chatId,
`🔄 <b>BULK RENEW COMPLETE</b>
━━━━━━━━━━━━━━━━━━
📦 App: <b>${escHtml(app.display_name)}</b>
👥 Users renewed: <b>${count}</b>
📅 New expiry: <b>${fmtExpiry(expiry)}</b>
✅ All active users have been extended.`, 'HTML');
    } catch (e) { await sendMessage(token, chatId, `❌ Error: ${e.message}`); }
    return;
  }

  // /check <device_id> [app_id]
  ...
  // /pending [app_id] — list pending activation requests
  if (/^\/pending/i.test(text)) {
    const parts = text.split(/\s+/);
    const filterApp = parts[1] || null;
    try {
      const rows = filterApp
        ? (await env.DB.prepare("SELECT * FROM pending_activations WHERE app_id=? ORDER BY created_at DESC LIMIT 30").bind(filterApp).all()).results || []
        : (await env.DB.prepare("SELECT * FROM pending_activations ORDER BY created_at DESC LIMIT 30").all()).results || [];
      if (!rows.length) {
        await sendMessage(token, chatId, `📭 <b>No pending requests${filterApp ? ` for ${escHtml(filterApp)}` : ''}</b>`, 'HTML');
        return;
      }
      // Build app_id → tg_command map
      const allApps = (await env.DB.prepare("SELECT app_id, tg_command FROM apps").all()).results || [];
      const cmdMap = {};
      for (const a of allApps) cmdMap[a.app_id] = a.tg_command || a.app_id;

      let msg = `⏳ <b>PENDING REQUESTS</b> (${rows.length})\n━━━━━━━━━━━━━━━━━━\n\n`;
      for (const r of rows) {
        const age = Math.floor((Date.now() - r.created_at) / 60000);
        const cmd = cmdMap[r.app_id] || r.app_id;
        msg += `👤 @${escHtml(r.telegram_username)} (<code>${r.telegram_user_id}</code>)\n`;
        msg += `📦 App: <b>${escHtml(r.app_id)}</b>\n`;
        msg += `📱 Device: <code>${escHtml(r.device_id)}</code>\n`;
        msg += `🕐 ${age}m ago\n`;
  ...
  // /clearapp [app_id] — clear one app's data
  if (/^\/clearapp/i.test(text)) {
    try {
      const parts = text.split(/\s+/);
      const directAppId = parts[1] || null;
      const apps = (await env.DB.prepare("SELECT app_id, display_name FROM apps ORDER BY display_name").all()).results || [];
      if (!apps.length) {
        await sendMessage(token, chatId, '📭 <b>No apps registered yet.</b>', 'HTML');
        return;
      }
      if (directAppId) {
        // Direct: /clearapp <app_id> — skip picker, go straight to confirm
        const app = apps.find(a => a.app_id === directAppId);
  ...
  if (/^\/stats/i.test(text)) {
    try {
      ...
      for (const r of all) {
        ...
        if (!r.is_active) { revoked++; byApp[r.app_id].revoked++; }
        else if (isExpired(r.expiry)) { expired++; byApp[r.app_id].expired++; }
        else { active++; byApp[r.app_id].active++; }
      }
      let msg =
`📊 <b>VIP SYSTEM STATS</b>
━━━━━━━━━━━━━━━━━━
...
⛔ Revoked: <b>${revoked}</b>`;
      ...
      const kb = { inline_keyboard: [[
        { text: '📋 Devices',    callback_data: 'ls:all:1'  },
        { text: '📂 Full List',  callback_data: 'lsa:all:1' },
      ]]};
  ...
  // /clear_database — full wipe with double confirmation
...
export default {
  async fetch(request, env) {
    ...
    try { await initDB(env.DB); } catch (e) { return json({ error: 'DB init failed', detail: e.message }, 500); }
    // ── Bind env secrets to module-level globals ─────────────────
    BOT_TOKEN        = env.BOT_TOKEN        || '';
    ADMIN_SECRET     = env.ADMIN_SECRET     || '';
    GROUP_ID_DEFAULT = parseInt(env.GROUP_ID    || '0') || 0;
    CHANNEL_ID       = parseInt(env.CHANNEL_ID  || '0') || 0;
    MUNO_EMAIL       = env.MUNO_EMAIL       || '';
    MUNO_PASS        = env.MUNO_PASS        || '';
    MUNO_API_KEY     = env.MUNO_API_KEY     || '';
    APP_HMAC_SECRET  = env.APP_HMAC_SECRET  || '';
    RESP_SIGN_SECRET = env.RESP_SIGN_SECRET || '';
    ...
    // VIP token  GET /api/vip/token?device_id=XXX&app_id=XXX
    if (path === '/api/vip/token' && method === 'GET') {
      ...
      const tkApp = await env.DB.prepare("SELECT app_secret FROM apps WHERE app_id=?").bind(appId).first();
      if (!validateAppSecret(tkApp, tkSec)) return json({ error: 'forbidden' }, 403);
      const token = await freshToken(env.DB, appId, deviceId);
      return json({ token });
    }

    // VIP check  POST /api/vip/check  {device_id, app_id}
    if (path === '/api/vip/check' && method === 'POST') {
      try {
        ...
        let _chkApp = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(app_id).first();
        if (!_chkApp) _chkApp = await env.DB.prepare("SELECT * FROM apps WHERE pkg_suffix != '' AND pkg_suffix=? LIMIT 1").bind(app_id.toLowerCase()).first();
        if (!_chkApp) _chkApp = await env.DB.prepare("SELECT * FROM apps WHERE instr(app_id,?) > 0 OR instr(?,app_id) > 0 ORDER BY length(app_id) DESC LIMIT 1").bind(app_id, app_id).first();
      ...
      } catch { return json({ active: false }); }
    }

    // Legacy check  GET /api/check?device_id=XXX[&app_id=XXX]
    if (path === '/api/check' && method === 'GET') {
      const deviceId = url.searchParams.get('device_id') || '';
      const appId = url.searchParams.get('app_id') || '';
      if (!deviceId) return json({ error: 'missing device_id' }, 400);
      if (appId) {
        let legApp = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(appId).first();
        if (!legApp) legApp = await env.DB.prepare("SELECT * FROM apps WHERE pkg_suffix != '' AND pkg_suffix=? LIMIT 1").bind(appId.toLowerCase()).first();
        if (!legApp) legApp = await env.DB.prepare("SELECT * FROM apps WHERE instr(app_id,?) > 0 OR instr(?,app_id) > 0 ORDER BY length(app_id) DESC LIMIT 1").bind(appId, appId).first();
        ...
        const row = await env.DB.prepare("SELECT expiry, is_active, warn_sent FROM app_activations WHERE app_id=? AND device_id=?").bind(resolvedLegId, deviceId).first();
        if (!row) return json({ active: false, registered: false, ts: Date.now() });
        const expired = isExpired(row.expiry);
        const near_expiry = !expired && isNearExpiry(row.expiry);
        return json({ active: row.is_active===1&&!expired, expired, registered: true, expiry: row.expiry, ts: Date.now(), near_expiry, warn_needed: near_expiry && !row.warn_sent });
      }
    ...
    // Direct VIP dialog page — served to non-Telegram hooked apps by package name
    if (path === '/vip/direct' && method === 'GET') {
      const pkg      = url.searchParams.get('pkg') || '';
      const deviceId = url.searchParams.get('device_id') || '';
      const app = pkg ? await env.DB.prepare("SELECT * FROM apps WHERE package_name=?").bind(pkg).first() : null;
    ...
    // Direct check — POST {device_id, pkg, nonce, ts, sig}
    if (path === '/api/vip/check-direct') {
      ...
      // ── Verify request HMAC — reject spoofed/replayed checks ──────
      const hmacOk = await verifyRequestHmac({ device_id, nonce, ts, sig, pkg });
      if (!hmacOk) return json({ active: false, reason: 'invalid_request' }, 403);

      // ── Verify package name matches the registered app ─────────────
      if (pkg !== 'com.adiza.moviezbox') return json({ active: false, reason: 'invalid_pkg' }, 403);
      ...
      if (!row || !row.is_active) {
        const respTs = Math.floor(Date.now() / 1000);
        const respNonce = nonce;
        const rsig = await signResponse({ active: false, ts: respTs, nonce: respNonce });
        return json({ active: false, ts: respTs, nonce: respNonce, sig: rsig });
      }
      if (isExpired(row.expiry)) {
        const respTs = Math.floor(Date.now() / 1000);
        const rsig = await signResponse({ active: false, ts: respTs, nonce });
        return json({ active: false, expired: true, ts: respTs, nonce, sig: rsig });
      }

      // ── Group membership enforcement ──────────────────────────────
      if (BOT_TOKEN && GROUP_ID_DEFAULT && row.telegram_user_id) {
        try {
          const gmResp = await fetch(
            `https://api.telegram.org/bot${BOT_TOKEN}/getChatMember?chat_id=${GROUP_ID_DEFAULT}&user_id=${row.telegram_user_id}`,
            { cf: { cacheEverything: false } }
          );
          if (gmResp.ok) {
            const gm = await gmResp.json();
            if (gm.ok) {
              const status = gm.result?.status || '';
              const inGroup = ['member','administrator','creator','restricted'].includes(status);
              if (!inGroup) {
                const respTs = Math.floor(Date.now() / 1000);
                const rsig = await signResponse({ active: false, ts: respTs, nonce });
                return json({ active: false, reason: 'left_group',
                  telegram_username: row.telegram_username || '',
                  ts: respTs, nonce, sig: rsig });
              }
            }
          }
        } catch (_) { /* network error — fail open so the app still works */ }
      }

      // ── Sign the success response ──────────────────────────────────
      const respTs = Math.floor(Date.now() / 1000);
      const rsig = await signResponse({ active: true, ts: respTs, nonce });
      return json({ active: true, expiry: row.expiry,
        expiry_ms: row.expiry ? new Date(row.expiry).getTime() : 0,
        ts: respTs, nonce, sig: rsig });
    }

    // Warn-ack: marks that device has seen the 2-day expiry warning
    if (path === '/api/warn/ack' && method === 'GET') {
      const wDeviceId = url.searchParams.get('device_id') || '';
      const wAppId    = url.searchParams.get('app_id')    || '';
      if (wDeviceId && wAppId) {
        let wApp = await env.DB.prepare("SELECT app_id FROM apps WHERE app_id=?").bind(wAppId).first();
        if (!wApp) wApp = await env.DB.prepare("SELECT app_id FROM apps WHERE instr(app_id,?) > 0 OR instr(?,app_id) > 0 ORDER BY length(app_id) DESC LIMIT 1").bind(wAppId, wAppId).first();
    ...
    if (path === '/api/status' && method === 'GET') {
      ...
      if (appId) {
        const row = await env.DB.prepare("SELECT is_active, expiry FROM app_activations WHERE app_id=? AND device_id=?").bind(appId, deviceId).first();
    ...
    // Spinner gate — shown immediately by DEX before dialog loads
...
(async function(){
  ...
  window.location.replace("${spDialogUrl}");
    ...
    if (path === '/api/dialog' && method === 'GET') {
      ...
      if (appId) {
        // 1) Exact match
        app = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(appId).first();
        // 2) pkg_suffix exact match (admin-set package last segment)
        if (!app) {
          app = await env.DB.prepare(
            "SELECT * FROM apps WHERE pkg_suffix != '' AND pkg_suffix=? LIMIT 1"
          ).bind(appId.toLowerCase()).first();
        }
        // 3) Fuzzy fallback: stored app_id is a substring of provided, or provided is a substring of stored
        if (!app) {
          app = await env.DB.prepare(
            "SELECT * FROM apps WHERE instr(app_id,?) > 0 OR instr(?,app_id) > 0 ORDER BY length(app_id) DESC LIMIT 1"
          ).bind(appId, appId).first();
        }
        if (app) {
          resolvedAppId = app.app_id;
          appName = app.display_name; command = app.tg_command; groupLink = app.group_link;
        ...
        // Generate token under the resolved app_id so check/activate works
        const tkn = await freshToken(env.DB, resolvedAppId, deviceId);
        activationCode = `${deviceId}.${tkn}`;
      }
      if (app && app.dialog_variant === 'matrix') {
        return html(buildMatrixDialogHTML(deviceId, workerUrl, resolvedAppId, appName, app.group_link || 'https://t.me/+rvI8wgFwCiA3NWJk', dlgAppSecret));
      }
    ...
    if (path === '/api/admin/storage') {
      ...
      const apps = (await env.DB.prepare("SELECT app_id, display_name FROM apps").all()).results || [];
      const appMap = {};
      for (const a of apps) appMap[a.app_id] = a.display_name;
    ...
    if (path === '/api/admin/stats') {
      ...
      const q = appIdFilter
        ? "SELECT COUNT(*) as c FROM app_activations WHERE app_id=?"
        : "SELECT COUNT(*) as c FROM app_activations";
      const bind = appIdFilter ? [appIdFilter] : [];
      const total = appIdFilter
        ? await env.DB.prepare(q).bind(...bind).first()
        : await env.DB.prepare(q).first();
      const active = appIdFilter
        ? await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=1 AND app_id=? AND (expiry='2080' OR datetime(expiry)>datetime('now'))").bind(appIdFilter).first()
        : await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=1 AND (expiry='2080' OR datetime(expiry)>datetime('now'))").first();
      const expired = appIdFilter
        ? await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=1 AND app_id=? AND expiry != '2080' AND datetime(expiry) <= datetime('now')").bind(appIdFilter).first()
        : await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=1 AND expiry != '2080' AND datetime(expiry) <= datetime('now')").first();
      const revoked = appIdFilter
        ? await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=0 AND app_id=?").bind(appIdFilter).first()
        : await env.DB.prepare("SELECT COUNT(*) as c FROM app_activations WHERE is_active=0").first();
      // Per-app breakdown for dashboard
      let by_app = [];
      if (!appIdFilter) {
        const appRows = (await env.DB.prepare("SELECT app_id, display_name FROM apps ORDER BY created_at DESC").all()).results || [];
    ...
    if (path === '/api/admin/users') {
      ...
      const users = (rows.results || []).map(r => ({
        ...r, status: r.is_active===0 ? 'revoked' : isExpired(r.expiry) ? 'expired' : 'active'
      }));
      return json({ users, page });
    }

    // Get / save global settings (group_id etc.)
    ...
    if (path === '/api/admin/apps' && method === 'GET') {
      ...
      let apps;
      if (srcFilter === 'direct') {
        // Direct tab: only Hook-B apps (dialog_variant='matrix' or has package_name)
        apps = (await env.DB.prepare(
          "SELECT a.*, " +
          "(SELECT COUNT(*) FROM app_activations aa WHERE aa.app_id=a.app_id AND aa.is_active=1 AND (aa.expiry='2080' OR datetime(aa.expiry)>datetime('now')) AND aa.source='direct') as active_count, " +
          "(SELECT COUNT(*) FROM app_activations aa2 WHERE aa2.app_id=a.app_id AND aa2.source='direct') as total_count " +
          "FROM apps a WHERE a.dialog_variant='matrix' OR (a.package_name IS NOT NULL AND a.package_name!='') ORDER BY a.created_at DESC"
        ).all()).results || [];
      } else {
        // Telegram tab: only non-matrix apps
        apps = (await env.DB.prepare(
          "SELECT a.*, " +
          "(SELECT COUNT(*) FROM app_activations aa WHERE aa.app_id=a.app_id AND aa.is_active=1 AND (aa.expiry='2080' OR datetime(aa.expiry)>datetime('now')) AND (aa.source IS NULL OR aa.source='telegram')) as active_count, " +
          "(SELECT COUNT(*) FROM app_activations aa2 WHERE aa2.app_id=a.app_id AND (aa2.source IS NULL OR aa2.source='telegram')) as total_count " +
          "FROM apps a WHERE a.dialog_variant!='matrix' OR a.dialog_variant IS NULL ORDER BY a.created_at DESC"
        ).all()).results || [];
      }
      return json({ apps });
    }
    ...
    if (path === '/api/admin/apps/register' && method === 'POST') {
      const body = await request.json();
      const { display_name, app_id: providedAppId, package_name: providedPkg, expiry_default = '2080', group_link = '', app_secret = '', dialog_variant = 'reversal', pkg_id = '', pkg_suffix = '' } = body;
      if (!display_name) return json({ error: 'display_name required' }, 400);
      // Use provided app_id if given, otherwise derive from display_name
      const appId = (providedAppId || '').trim() || display_name.toLowerCase().replace(/[^a-z0-9]/g, '');
      const tgCommand = appId;
      // Use provided package_name if given, then pkg_id, then pkg_suffix
      const pkgIdClean = (providedPkg || pkg_id || '').toLowerCase().trim();
      const resolvedSuffix = pkgIdClean || pkg_suffix.toLowerCase().trim();
      await env.DB.prepare("INSERT OR REPLACE INTO apps (app_id, display_name, tg_command, expiry_default, group_link, app_secret, dialog_variant, pkg_suffix, package_name, created_at) VALUES (?,?,?,?,?,?,?,?,?,?)")
        .bind(appId, display_name, tgCommand, expiry_default, group_link, app_secret, dialog_variant, resolvedSuffix, pkgIdClean, Date.now()).run();
      // Auto-broadcast announcement — skip for Matrix (no-Telegram) apps
      if (dialog_variant !== 'matrix') {
        const token = BOT_TOKEN;
        const groupId = await getGroupId(env);
        if (token && groupId) {
          const app = { app_id: appId, display_name, tg_command: tgCommand };
          const channelId = getChannelId();
          const photo = await getNextBannerPhoto(env.DB);
          const caption = buildAnnouncementMessage(app).replace(/\r/g, '');
          await sendPhoto(token, channelId, photo, caption, ANNOUNCE_KB, 'Markdown');
        }
      }
      return json({ ok: true, app_id: appId, tg_command: tgCommand, app_secret, dialog_variant });
    }

    // Delete app — cascade wipes ALL related data
    if (path === '/api/admin/apps' && method === 'DELETE') {
      ...
      await env.DB.prepare("DELETE FROM apps WHERE app_id=?").bind(appId).run();
    ...
    if (path === '/api/admin/broadcast' && method === 'POST') {
      ...
      const app = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(app_id).first();
    ...
    if (path === '/api/admin/test-bot' && method === 'POST') {
      ...
      const tgResp = await (await sendMessage(token, groupId, '🤖 <b>REVERSAL X</b> — bot connection test ✅\nIf you see this, announcements are working!', 'HTML')).json();
    ...
    if (path === '/api/admin/activate' && method === 'POST') {
      ...
      const expiry = parseExpiry(rawExp);
      if (app_id) {
        const existingApp = await env.DB.prepare("SELECT app_id FROM apps WHERE app_id=?").bind(app_id).first();
        if (!existingApp) {
          const dName = app_display_name || app_id;
          await env.DB.prepare(
            "INSERT OR IGNORE INTO apps (app_id, display_name, tg_command, expiry_default, group_link, app_secret, dialog_variant, created_at) VALUES (?,?,?,?,?,?,?,?)"
          ).bind(app_id, dName, app_id, rawExp, '', '', 'matrix', Date.now()).run();
        }
    ...
    // Direct users list — grouped by device_id (person-first view)
    if (path === '/api/admin/direct/users' && method === 'GET') {
      const rows = (await env.DB.prepare(
        "SELECT device_id, telegram_username, " +
        "COUNT(*) as total_apps, " +
        "SUM(CASE WHEN is_active=1 AND (expiry='2080' OR datetime(expiry)>datetime('now')) THEN 1 ELSE 0 END) as active_apps, " +
        "MAX(created_at) as last_activated " +
        "FROM app_activations WHERE source='direct' " +
        "GROUP BY device_id ORDER BY last_activated DESC"
      ).all()).results || [];
      return json({ users: rows });
    }

    // Direct user-apps — all apps a specific device_id is activated on
    if (path === '/api/admin/direct/user-apps' && method === 'GET') {
      const did = url.searchParams.get('device_id') || '';
      if (!did) return json({ apps: [] });
      const rows = (await env.DB.prepare(
        "SELECT aa.app_id, aa.device_id, aa.telegram_username, aa.expiry, aa.is_active, aa.created_at, " +
        "a.display_name, a.package_name " +
        "FROM app_activations aa LEFT JOIN apps a ON aa.app_id=a.app_id " +
        "WHERE aa.device_id=? AND aa.source='direct' ORDER BY aa.created_at DESC"
      ).bind(did).all()).results || [];
      return json({ apps: rows });
    }

    // Direct clear-users — POST {app_id} — deletes only source='direct' activations, keeps app record
    if (path === '/api/admin/direct/clear-users' && method === 'POST') {
      let _dcuBody; try { _dcuBody = await request.json(); } catch { return json({ error: 'Invalid JSON body' }, 400); }
    ...
    // Direct activate — activates by package_name + device_id with source='direct'
    if (path === '/api/admin/activate-direct' && method === 'POST') {
      let _dab; try { _dab = await request.json(); } catch { return json({ error: 'Invalid JSON body' }, 400); }
      ...
      let app = await env.DB.prepare("SELECT app_id FROM apps WHERE package_name=?").bind(package_name.trim()).first();
      if (!app) {
        // Auto-register the app so Direct Activate works without pre-registration
        const autoId = package_name.trim().replace(/[^a-z0-9]/gi, '_').toLowerCase().slice(0, 40);
        await env.DB.prepare(
          "INSERT OR IGNORE INTO apps (app_id, display_name, package_name, tg_command, expiry_default, group_link, app_secret, dialog_variant, created_at) VALUES (?,?,?,?,?,?,?,'matrix',?)"
        ).bind(autoId, app_name || package_name, package_name.trim(), '', '2080', '', '', Date.now()).run();
        app = await env.DB.prepare("SELECT app_id FROM apps WHERE package_name=?").bind(package_name.trim()).first();
        if (!app) return json({ error: 'Failed to register app — try again' }, 500);
      }
      const expiry = parseExpiry(rawExp);
      await env.DB.prepare(
    ...
    if (path === '/api/admin/db') {
      ...
      return json({ app_activations: activations.results, apps: apps.results, legacy_activations: legacy.results });
    }
    ...
    // ── Update telegram username for a device ─────────────────
    if (path === '/api/admin/set-username' && method === 'POST') {
      ...
      if (app_id) {
        await env.DB.prepare("UPDATE app_activations SET telegram_username=? WHERE app_id=? AND device_id=?")
          .bind(username, app_id, device_id).run();
      } else {
        await env.DB.prepare("UPDATE app_activations SET telegram_username=? WHERE device_id=?")
          .bind(username, device_id).run();
      }
      return json({ ok: true, device_id, username });
    }

    // ── Bulk activate (multiple device IDs at once) ───────────
    ...
    if (path === '/api/admin/pending' && method === 'GET') {
      const appId = url.searchParams.get('app_id') || '';
      const rows = appId
        ? (await env.DB.prepare("SELECT * FROM pending_activations WHERE app_id=? ORDER BY created_at DESC").bind(appId).all()).results || []
        : (await env.DB.prepare("SELECT * FROM pending_activations ORDER BY created_at DESC").all()).results || [];
    ...
    // ── Chart data — 7-day activations ───────────────────────
    if (path === '/api/admin/chart-data' && method === 'GET') {
      const nowMs = Date.now();
      const daily = [];
      for (let i = 6; i >= 0; i--) {
        const dayStart = nowMs - (i + 1) * 86400000;
        const dayEnd   = nowMs - i * 86400000;
        const r = await env.DB.prepare(
          "SELECT COUNT(*) as c FROM app_activations WHERE created_at >= ? AND created_at < ?"
        ).bind(dayStart, dayEnd).first();
        const label = new Date(dayStart + 43200000).toLocaleDateString('en-US', { month:'short', day:'numeric', timeZone:'UTC' });
    ...
    if (path === '/api/admin/apps' && method === 'PATCH') {
      const body = await request.json();
      const { app_id, display_name, expiry_default, dialog_variant } = body;
      if (!app_id) return json({ error: 'app_id required' }, 400);
      const existing = await env.DB.prepare("SELECT * FROM apps WHERE app_id=?").bind(app_id).first();
      if (!existing) return json({ error: 'app not found' }, 404);
      const newName    = display_name    !== undefined ? display_name    : existing.display_name;
      const newExpiry  = expiry_default  !== undefined ? expiry_default  : existing.expiry_default;
      const newVariant = dialog_variant  !== undefined ? dialog_variant  : existing.dialog_variant;
      await env.DB.prepare(
        "UPDATE apps SET display_name=?, expiry_default=?, dialog_variant=? WHERE app_id=?"
      ).bind(newName, newExpiry, newVariant, app_id).run();
      return json({ ok:true, app_id, display_name:newName, expiry_default:newExpiry, dialog_variant:newVariant });
    }

    
    // ════════════════════════════════════════════════════════════
    //  UGANDA MUNOWATCH ROUTES
    // ════════════════════════════════════════════════════════════

    // GET / — health (also handles the munowatch base route)
    if (path === '/' || path === '') {
      return new Response(JSON.stringify({ status: 'ok', worker: 'uganda-munowatch-v1.0', ts: Date.now() }), {
        headers: { 'Content-Type': 'application/json', ...CORS }
      });
    }

    // GET /browse — VJ + genre listing (no auth needed)
    if (path === '/browse') {
      try {
    ...
    // GET /grid — movie grid by VJ or genre
    if (path === '/grid') {
      try {
        const pipeType = url.searchParams.get('pipe_type') || 'p';
        const pipeId   = url.searchParams.get('pipe_id')   || '4';
        const page     = url.searchParams.get('page')      || '1';
        const r = await munoAuthedGet(`/grid?pipe_type=${encodeURIComponent(pipeType)}&pipe_id=${encodeURIComponent(pipeId)}&page=${encodeURIComponent(page)}`);
        const movies = munoParseMovies(await r.text());
        return new Response(JSON.stringify({ movies, page: parseInt(page, 10), hasMore: movies.length >= 24 }), { headers: { 'Content-Type': 'application/json', ...CORS } });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: { 'Content-Type': 'application/json', ...CORS } }); }
    }
    ...
    if (path === '/search') {
      try {
        ...
        const r = await munoAuthedGet(`/search?q=${encodeURIComponent(q)}&page=${encodeURIComponent(page)}`);
        const movies = munoParseMovies(await r.text());
        return new Response(JSON.stringify({ movies, q, page: parseInt(page, 10), hasMore: movies.length >= 24 }), { headers: { 'Content-Type': 'application/json', ...CORS } });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: { 'Content-Type': 'application/json', ...CORS } }); }
    }

    // GET /movies — alias for /search (all movies, no filter)
    if (path === '/movies') {
      try {
        const page = url.searchParams.get('page') || '1';
        const r = await munoAuthedGet(`/grid?pipe_type=p&pipe_id=4&page=${encodeURIComponent(page)}`);
        const movies = munoParseMovies(await r.text());
        return new Response(JSON.stringify({ movies, page: parseInt(page, 10), hasMore: movies.length >= 24 }), { headers: { 'Content-Type': 'application/json', ...CORS } });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: { 'Content-Type': 'application/json', ...CORS } }); }
    }

    // GET /stream — get CDN video URL
    if (path === '/stream') {
      try {
        ...
        const uid = '1';
        const h = {
          'User-Agent': MUNO_UA, 'Accept': '*/*', 'Accept-Encoding': 'identity',
        ...
        if (dlResp.ok) {
          ...
          if (data && data.playingurl) return new Response(JSON.stringify({ url: data.playingurl, title: data.title||'', vj: data.vj||'', size: data.size||'', duration: data.duration||'', image: data.image||'', vid }), { headers: { 'Content-Type': 'application/json', ...CORS } });
        }
        ...
        if (viewResp.ok) {
          ...
          if (data && data.nextUrl) return new Response(JSON.stringify({ url: data.nextUrl, title: data.vtitle||'', vid }), { headers: { 'Content-Type': 'application/json', ...CORS } });
        }
        ...
        if (previewResp.ok) {
          ...
          if (data) {
            const findUrl = (obj) => {
              ...
              if (typeof obj === 'object' && obj !== null) { for (const v of Object.values(obj)) { const found = findUrl(v); if (found) return found; } }
              return null;
            };
            const videoUrl = findUrl(data);
            if (videoUrl) return new Response(JSON.stringify({ url: videoUrl, vid }), { headers: { 'Content-Type': 'application/json', ...CORS } });
          }
        }
        return new Response(JSON.stringify({ error: 'Stream URL not found', vid }), { status: 404, headers: { 'Content-Type': 'application/json', ...CORS } });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: { 'Content-Type': 'application/json', ...CORS } }); }
    }
